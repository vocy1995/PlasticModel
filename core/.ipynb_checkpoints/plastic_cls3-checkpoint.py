...
class PlasticModel(nn.Module):
    def __init__(self, kernel_size):
        ...
        conv4 = nn.Conv2d(1, 128, kernel_size=(32, self.kernel_size), stride=1)
        pool4 = nn.MaxPool2d(kernel_size=(max_kernel_size - 32, 1))
        
        conv5 = nn.Conv2d(1, 128, kernel_size=(64, self.kernel_size), stride=1)
        pool5 = nn.MaxPool2d(kernel_size=(max_kernel_size - 64, 1))
        ...
        conv4 = self.init_layer(conv4)
        conv5 = self.init_layer(conv5)
        ...
        self.conv4_module = nn.Sequential(conv4, batch1, nn.ReLU(), pool4)
        self.conv5_module = nn.Sequential(conv5, batch1, nn.ReLU(), pool5)
        
    def forward(self, x):
        out1 = self.conv1_module(x)
        out2 = self.conv2_module(x)
        out3 = self.conv3_module(x)
        out4 = self.conv4_module(x)
        out5 = self.conv5_module(x)
